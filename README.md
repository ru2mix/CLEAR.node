...
1. Устанавливаем сервер как угодно
2. Запускаем утилиту CLEAR.bat
3. Регистрируем УЗ CLEAR, нужно зайти во вкладку Templates/Шаблоны и там нажать регистрация, если УЗ уже есть, ничего делать не нужно.
4. Перходим во вкладку "Сервер CLEAR.node" в боковом меню.
5. Нажимаем +
6. Прописываем ip и порт сервера в формате 192.168.1.1:8001, ваша УЗ получит права администратора, первая УЗ подключенная к серверу всегда администратор.
7. Если нужно импортировать подключения из локальной БД в облачную CLEAR.node, то нужно перейти на вкладку Настройки сервера, там есть кнопка импорт.
8. Для каждой группы прав нужно отредактировать видимость папок.
